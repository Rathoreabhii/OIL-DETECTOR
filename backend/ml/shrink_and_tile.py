...
def tile_scene(img: np.ndarray, mask: np.ndarray, tile_size: int = 512, step: int = 448):
    """Slice 2D image and mask into overlapping 512x512 tiles."""
    h, w = img.shape
    tiles = []
...
def process_dataset(max_scenes: int = MAX_SCENES_TO_PROCESS):
    ...
    # 1. Unpack if 7z exists
    extract_7z_archives()

    # 2. Find image & mask files
    # Check common extracted folder names
    img_candidates = list(ZENODO_RAW_DIR.rglob("*images*/*.tif*")) + list(ZENODO_RAW_DIR.rglob("*images*/*.png"))
    mask_candidates = list(ZENODO_RAW_DIR.rglob("*mask*/*.tif*")) + list(ZENODO_RAW_DIR.rglob("*mask*/*.png"))

    if not img_candidates:
        # Fallback to search any tiff/png in zenodo folder
        img_candidates = [p for p in ZENODO_RAW_DIR.rglob("*.tif*") if "mask" not in p.name.lower()]
        mask_candidates = [p for p in ZENODO_RAW_DIR.rglob("*.tif*") if "mask" in p.name.lower()] + \
                          [p for p in ZENODO_RAW_DIR.rglob("*.png") if "mask" in p.name.lower()]

    print(f"[*] Found {len(img_candidates)} candidate images and {len(mask_candidates)} masks in {ZENODO_RAW_DIR}")

    if not img_candidates:
        print("[!] No images found! Place the downloaded Zenodo files in:")
        print(f"    {ZENODO_RAW_DIR}")
        return

    # Build pairs based on stem match
    mask_map = {m.stem.replace("_mask", "").replace("mask_", ""): m for m in mask_candidates}
    pairs = []
    for img_path in img_candidates:
        key = img_path.stem.replace("_image", "").replace("image_", "")
        if key in mask_map:
    ...
    for idx, (img_p, mask_p) in enumerate(tqdm(pairs, desc="Tiling scenes")):
        try:
            ...
            for t_idx, (t_img, t_mask) in enumerate(tiles):
                ...
                if save_tile:
                    tile_name = f"scene_{idx:04d}_tile_{t_idx:03d}.png"
